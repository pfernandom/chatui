...
func (t *TextArea) Render(app *App) *Element {
	...
	if t.text.Get() == "" && t.placeholder != "" && !t.focused.Get() {
		...
		// Only emit one flex child per visible row. Previously every wrapped line was
		// added even when maxHeight capped layout height, so extra rows drew past the
		// bottom border and corrupted the frame.
...
// wrapLineLimit is the maximum runes per soft-wrapped line. With a border, text is
// laid out in the inner content rect (outer width minus two border columns).
func (t *TextArea) wrapLineLimit() int {
	if t.width <= 0 {
		return 0
	}
	if t.border == BorderNone {
